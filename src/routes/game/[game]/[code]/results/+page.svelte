<script lang="ts">
	import * as Table from '$lib/components/ui/table';
</script>

<section class="mt-3 grid place-items-center">
	<div class="min-h-screen">
		<div class="max-w-lg text-center">
			<h1 class="text-4xl font-bold leading-5">Game Results!</h1>
			<p class="py-4 text-sm opacity-90">
				Lorem ipsum dolor, sit amet consectetur adipisicing elit. Corporis delectus voluptatum
				quibusdam dicta! Tempore incidunt eius enim nihil beatae exercitationem, itaque in sunt
				doloremque nemo optio, tempora numquam fugiat nobis?
			</p>
		</div>
		<div>
			<Table.Root>
				<Table.Caption>Lorem ipsum dolor, sit amet consectetur adipisicing elit</Table.Caption>
				<Table.Header>
					<Table.Row>
						<Table.Head class="text-left">Pos.</Table.Head>
						<Table.Head class="w-[100px]">Player</Table.Head>
						<Table.Head>Corrects</Table.Head>
						<Table.Head>Wrongs</Table.Head>
						<Table.Head class="text-right">Score</Table.Head>
					</Table.Row>
				</Table.Header>
				<Table.Body>
					<Table.Row>
						<Table.Cell class="font-extrabold"># 1</Table.Cell>
						<Table.Cell class="font-medium">DOPE</Table.Cell>
						<Table.Cell>5</Table.Cell>
						<Table.Cell>0</Table.Cell>
						<Table.Cell class="text-right">1323</Table.Cell>
					</Table.Row>
				</Table.Body>
			</Table.Root>
		</div>
		<hr class="my-6" />
		<div class="mt-7 grid w-full place-items-center">
			<div class="mb-5 flex gap-4">
				<div class="my-4 rounded-full bg-white/5">
					<svg
						xmlns="http://www.w3.org/2000/svg"
						fill="none"
						viewBox="0 0 24 24"
						stroke-width="1.5"
						stroke="currentColor"
						class="m-4 aspect-square h-20 w-auto"
					>
						<path
							stroke-linecap="round"
							stroke-linejoin="round"
							d="M16.5 18.75h-9m9 0a3 3 0 0 1 3 3h-15a3 3 0 0 1 3-3m9 0v-3.375c0-.621-.503-1.125-1.125-1.125h-.871M7.5 18.75v-3.375c0-.621.504-1.125 1.125-1.125h.872m5.007 0H9.497m5.007 0a7.454 7.454 0 0 1-.982-3.172M9.497 14.25a7.454 7.454 0 0 0 .981-3.172M5.25 4.236c-.982.143-1.954.317-2.916.52A6.003 6.003 0 0 0 7.73 9.728M5.25 4.236V4.5c0 2.108.966 3.99 2.48 5.228M5.25 4.236V2.721C7.456 2.41 9.71 2.25 12 2.25c2.291 0 4.545.16 6.75.47v1.516M7.73 9.728a6.726 6.726 0 0 0 2.748 1.35m8.272-6.842V4.5c0 2.108-.966 3.99-2.48 5.228m2.48-5.492a46.32 46.32 0 0 1 2.916.52 6.003 6.003 0 0 1-5.395 4.972m0 0a6.726 6.726 0 0 1-2.749 1.35m0 0a6.772 6.772 0 0 1-3.044 0"
						/>
					</svg>
				</div>
				<div class="my-4 scale-125 rounded-full bg-white/5">
					<svg
						xmlns="http://www.w3.org/2000/svg"
						fill="none"
						viewBox="0 0 24 24"
						stroke-width="1.5"
						stroke="currentColor"
						class="m-4 aspect-square h-20 w-auto"
					>
						<path
							stroke-linecap="round"
							stroke-linejoin="round"
							d="M16.5 18.75h-9m9 0a3 3 0 0 1 3 3h-15a3 3 0 0 1 3-3m9 0v-3.375c0-.621-.503-1.125-1.125-1.125h-.871M7.5 18.75v-3.375c0-.621.504-1.125 1.125-1.125h.872m5.007 0H9.497m5.007 0a7.454 7.454 0 0 1-.982-3.172M9.497 14.25a7.454 7.454 0 0 0 .981-3.172M5.25 4.236c-.982.143-1.954.317-2.916.52A6.003 6.003 0 0 0 7.73 9.728M5.25 4.236V4.5c0 2.108.966 3.99 2.48 5.228M5.25 4.236V2.721C7.456 2.41 9.71 2.25 12 2.25c2.291 0 4.545.16 6.75.47v1.516M7.73 9.728a6.726 6.726 0 0 0 2.748 1.35m8.272-6.842V4.5c0 2.108-.966 3.99-2.48 5.228m2.48-5.492a46.32 46.32 0 0 1 2.916.52 6.003 6.003 0 0 1-5.395 4.972m0 0a6.726 6.726 0 0 1-2.749 1.35m0 0a6.772 6.772 0 0 1-3.044 0"
						/>
					</svg>
				</div>
				<div class="my-4 rounded-full bg-white/5">
					<svg
						xmlns="http://www.w3.org/2000/svg"
						fill="none"
						viewBox="0 0 24 24"
						stroke-width="1.5"
						stroke="currentColor"
						class="m-4 aspect-square h-20 w-auto"
					>
						<path
							stroke-linecap="round"
							stroke-linejoin="round"
							d="M16.5 18.75h-9m9 0a3 3 0 0 1 3 3h-15a3 3 0 0 1 3-3m9 0v-3.375c0-.621-.503-1.125-1.125-1.125h-.871M7.5 18.75v-3.375c0-.621.504-1.125 1.125-1.125h.872m5.007 0H9.497m5.007 0a7.454 7.454 0 0 1-.982-3.172M9.497 14.25a7.454 7.454 0 0 0 .981-3.172M5.25 4.236c-.982.143-1.954.317-2.916.52A6.003 6.003 0 0 0 7.73 9.728M5.25 4.236V4.5c0 2.108.966 3.99 2.48 5.228M5.25 4.236V2.721C7.456 2.41 9.71 2.25 12 2.25c2.291 0 4.545.16 6.75.47v1.516M7.73 9.728a6.726 6.726 0 0 0 2.748 1.35m8.272-6.842V4.5c0 2.108-.966 3.99-2.48 5.228m2.48-5.492a46.32 46.32 0 0 1 2.916.52 6.003 6.003 0 0 1-5.395 4.972m0 0a6.726 6.726 0 0 1-2.749 1.35m0 0a6.772 6.772 0 0 1-3.044 0"
						/>
					</svg>
				</div>
			</div>
			<h1 class="text-6xl font-bold leading-5 underline decoration-wavy underline-offset-8">
				You Won!
			</h1>
		</div>
	</div>
</section>
